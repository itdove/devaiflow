...
def complete_session(
    ...
    # Check if this is a new multi-project conversation (one conversation, multiple projects)
    if session.session_type == "development" and active_conv and active_conv.is_multi_project and active_conv.projects:
        console.print(f"\n[bold cyan]Processing {len(active_conv.projects)} projects:[/bold cyan]")

        # Iterate through all projects in the multi-project conversation
        for proj_name, proj_info in active_conv.projects.items():
            if not proj_info.project_path or not proj_info.branch:
                continue

            working_dir = Path(proj_info.project_path)
            ...
            current_git_branch = GitUtils.get_current_branch(working_dir)
            if current_git_branch != proj_info.branch:
                console.print(f"  [yellow]⚠[/yellow] Branch mismatch: expected '{proj_info.branch}', on '{current_git_branch}'")
            ...
            if GitUtils.has_uncommitted_changes(working_dir):
                ...
                if no_commit:
                    ...
                    should_commit = Confirm.ask(f"  Commit changes in {proj_name}?", default=True)
...
Co-Authored-By: Claude <noreply@anthropic.com>"""
            ...
            if pr_status and pr_status['state'] == 'open':
                ...
                if changed_files:
                    ...
                    if should_create:
                        # Create PR/MR for this project
                        pr_url = _create_pr_mr_for_project(session, proj_info, working_dir, session_manager)
...
def _create_pr_mr_for_project(session, proj_info, working_dir: Path, session_manager) -> Optional[str]:
    ...
    if not proj_info or not proj_info.branch:
        return None
    ...
    # Use project's base_branch as target branch
    target_branch = proj_info.base_branch
